...
<%
	...
	String date=component.getSYSTEMDATE();
		...
		event = event.initEvent(1, USER_ID,this.getClass().getSimpleName().replace("_002d", "-").replace("_", ".")+" policy number :"+request.getParameter("policynumber"));
		Application_Properties.SERIAL = 1;
		int x = component.getJdbcTemplate().update(event.strSQL(event));
		System.out.println("eventLog:"+x+"+ uid: "+USER_ID);
		...
		<legend><b>Report filter</b></legend>
		    ...
		    <table class="form" >
		        ...
		        <tr>
					...
					<td >
						<input type="text" id="fireEngineering_txtpolicynumber" name="fireEngineering_txtpolicynumber" style="width: 600px; height: 50px; font-size:18pt;" />					       	
					</td>
		        </tr>
		        ...
		        <tr>
		        	...
		        	<td>
				        <button class="btn btn-primary" id="btnExporttoPDF-Fire-And-Engineering" style="width: 600px; height: 50px; font-size:18pt;">
...
$('#btnExporttoPDF-Fire-And-Engineering').click(function(){
	...
	var v_policynumber=$('#fireEngineering_txtpolicynumber').val();
	var v_txtvattin=$('#fireEngineering_txtvattin').val();
	window.open("FireEngineer.htm?policynumber="+v_policynumber+ "&vattin="+v_txtvattin+"&reportType=1");
});
</script>
<script type="text/javascript">    
