<%@page import="sitha.rupp.service.SYS_DATEService"%>
<%@page import="sitha.rupp.service.rpt_0023_daily_claimServices"%>
<%@page import="sitha.rupp.model.rpt_0023_daily_claim_model"%>
<%@page import="sitha.rupp.configuration.Application_Properties"%>
<%@page import="sitha.rupp.service.PrincebankComponent"%>
<%@page import="sitha.rupp.model.SYS_EVENT_LOG"%>
<%@page import="java.util.List"%>
<%@page import="sitha.rupp.helper.ClsHelper"%>
<%@page import="java.util.ArrayList"%>
<%@page import="org.springframework.context.support.ClassPathXmlApplicationContext"%>
<%@page import="org.springframework.context.ApplicationContext"%>
<%@page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%
	ApplicationContext context=new ClassPathXmlApplicationContext("Spring_Bean.xml");
	PrincebankComponent component=PrincebankComponent.getInstance();
	String BRANCH_ID = (String) request.getParameter("BRANCH_ID");
	String USER_ID = component.USER_ID(request)+"";
	
	String fromDate = (String) request.getParameter("FromDate");
	String toDate = (String) request.getParameter("ToDate");
	String ProductCode=(String)request.getParameter("productCode");
	
	if(fromDate==null){
		fromDate = component.getSYSTEMDATE();
		//fromDate = "01-NOV-2020";
	}
	else{
		fromDate = (String) request.getParameter("FromDate");
	}
	if(toDate==null){
		toDate = component.getSYSTEMDATE();
	}
	else{
		toDate = (String) request.getParameter("ToDate");
	}
	
	
	out.print(fromDate + "---"+ toDate);
	out.print("ProductCode="+ProductCode);
	
	if(ProductCode == null){
		ProductCode = "2";
	}
	else{
		ProductCode = "2";
	}
	int uid = Integer.parseInt(USER_ID);
	SYS_EVENT_LOG event = new SYS_EVENT_LOG();
	event = event.initEvent(1, uid,this.getClass().getSimpleName().replace("_002d", "-").replace("_", ".") +", from date :" + fromDate + ", to date : " + toDate);
	Application_Properties.SERIAL = 1;
	int x = component.getJdbcTemplate().update(event.strSQL(event));
	System.out.println("eventLog:"+x+"+ uid: "+uid);
	
	Application_Properties.SERIAL=1;
	rpt_0023_daily_claimServices rptDailyClaim = new rpt_0023_daily_claimServices();
	List<rpt_0023_daily_claim_model> alist = new ArrayList<rpt_0023_daily_claim_model>();
	alist = rptDailyClaim.getDailyClaim(fromDate, toDate, "2");
%>

<script src="assets/alert/dialogbox.js"></script>
<script src="assets/choosen/chosen.jquery.js" type="text/javascript"></script>
<div style="height: 15px;"></div>
<table>
	<tr align="center">
		<td align="left" style="text-align:right; height: 50px; font-size:14pt;" >From date :</td>
		<td align="left" width="250px">
	        <div class="input-group datepicker-demo date rpt_0023_daily_claim_pa_hc_txtReportDate" data-date=""
				data-date-format="dd-M-yyyy">
		        <input type="text" style="width: 300px; height: 50px; font-size:18pt; text-align: right;" value="<%out.print(fromDate); %>" 
		        id="rpt_0023_daily_claim_pa_hc_txtDateFrom" name="rpt_0023_daily_claim_pa_hc_txtDateFrom" class="form-control" placeholder="dd-MM-yyyy" data-date-format="dd-M-yyyy">
		        <span class="input-group-addon"><i class="fa fa-calendar"></i></span>
		    </div>
		</td>
		
<!-- 		<td align="left" style="padding-left:3px; "> -->
<!-- 			 <button class="btn btn-primary" id="rpt_0023_daily_claim_pa_hc_view_pdf" style="width: 340px; height: 50px; font-size:18pt;"> -->
<!-- 				<i class="fa fa-search"></i> VIEW PDF -->
<!-- 			</button> -->
<!-- 		</td>			 -->
	</tr>
	<tr>
		<td align="left" style="text-align:right; height: 50px; font-size:14pt;">To date :</td>
		<td align="left" width="250px">
	        <div class="input-group datepicker-demo date rpt_0023_daily_claim_pa_hc_txtReportDate" data-date=""
				data-date-format="dd-M-yyyy">
		        <input type="text" style="width: 300px; height: 50px; font-size:18pt; text-align: right;" value="<%out.print(toDate); %>" 
		        id="rpt_0023_daily_claim_pa_hc_txtDateTo" name="rpt_0023_daily_claim_pa_hc_txtDateTo" class="form-control" placeholder="dd-MM-yyyy" data-date-format="dd-M-yyyy">
		        <span class="input-group-addon"><i class="fa fa-calendar"></i></span>
		    </div>
		</td>
<!-- 		<td align="left" style="padding-left:3px; "> -->
<!-- 			 <button class="btn btn-primary" id="rpt_0023_daily_claim_pa_hc_export_excel" style="width: 340px; height: 50px; font-size:18pt;"> -->
<!-- 				<i class="fa fa-search"></i> EXPORT EXCEL -->
<!-- 			</button> -->
<!-- 		</td> -->
	</tr>
	<tr>
		<td align="left" style="text-align:right; height: 50px; font-size:14pt;">Select Product</td>
		<td>
			<select id="rpt_0023_daily_claim_pa_hc_productCode" style="width: 100%; height: 50px; font-size:14pt;" disabled="disabled">
<!-- 				<option value="ALL">All</option> -->
		            <%	
					out.print(component.createOptionValue("SELECT ID,PRODUCT_NAME FROM APIREPORT.PRODUCTS ORDER BY 1", "ID", "PRODUCT_NAME", 2));		
					%>
			</select>
		</td>  
		<td align="left" style="padding-left:3px; ">
			 <button class="btn btn-primary" id="rpt_0023_daily_claim_pa_hc_refresh" style="width: 340px; height: 50px; font-size:18pt;">
				<i class="fa fa-search"></i> REFRESH
			</button>
		</td>     
	</tr>
	
</table>  
<hr>

<div class="preloading" style="left: 55%;margin-right: -45%;position: absolute;margin: 0px;transfrom:translate(-50%,-50%); ">
    <img src="loading/img/processing.gif" width="" />
</div>

<div class="col-xs-12 content" style="margin-top:0px; display: none;">
     <table style="width: 100%;" id="rpt_0023_daily_claim_pa_hc_datatable" class="table table-bordered table-hover display">                    	
        <thead>
           <tr>
               <th class="text-center">NO</th>
				<th class="text-center">AC_MONTH</th>
				<th class="text-center">Risk Type</th>
				<th class="text-center">Approval No. In System</th>
				<th class="text-center">Claim No</th>
				<th class="text-center">POLICY_NUMBER</th>
				<th class="text-center">INSURED_NAME</th>
				<th class="text-center">CLAIMANT</th>
				<th class="text-center">SUM_INSURED</th>
				<th class="text-center">Loss Location</th>
				<th class="text-center">HOSPITAL_NAME</th>
				<th class="text-center">Cause/Type of Loss</th>
				<th class="text-center">INCEPTION_DATE</th>
				<th class="text-center">EXPIRE_DATE</th>
				<th class="text-center">LOST_DATE</th>
				<th class="text-center">SETTLEMENT_DATE</th>
				<th class="text-center">LOSS_100</th>
				<th class="text-center">AMOUNT</th>
				<th class="text-center">CEDANT_PAID</th>
				<th class="text-center">OBLIGATORY</th>
				<th class="text-center">NARRATION_REMARK</th>
				<th class="text-center">CLAIM_STATUS_DESC</th>
				<th class="text-center">REASON_OF_CLAIM_STATUS</th>
				<th class="text-center">REPORTED_DATE</th>
				<th class="text-center">APPROVED_DATE</th>
				<th class="text-center">TO_DATE</th>
				<th class="text-center">OS_DAYS</th>
           </tr>
       <thead>    
       	   <tbody>
       		<% 
       		for(int i=0;i<alist.size();i++){
   				rpt_0023_daily_claim_model mdailyClaim=alist.get(i);
       		%>
       		<tr>
				<td align="center"><%out.print(mdailyClaim.getNo()==null?"":mdailyClaim.getNo()); %></td>
				<td align="center"><%out.print(mdailyClaim.getAc_month()==null?"":mdailyClaim.getAc_month()); %></td>
				<td align="center"><%out.print(mdailyClaim.getProduct()==null?"":mdailyClaim.getProduct()); %></td>
				<td align="center"><%out.print(mdailyClaim.getPapprh_number()==null?"":mdailyClaim.getPapprh_number()); %></td>
				<td align="center"><%out.print(mdailyClaim.getClaim_number()==null?"":mdailyClaim.getClaim_number()); %></td>
				<td align="center"><%out.print(mdailyClaim.getPolicy_number()==null?"":mdailyClaim.getPolicy_number()); %></td>
				<td align="center"><%out.print(mdailyClaim.getInsured_name()==null?"":mdailyClaim.getInsured_name()); %></td>
				<td align="center"><%out.print(mdailyClaim.getClaimant()==null?"":mdailyClaim.getClaimant()); %></td>
				<td align="center"><%out.print(ClsHelper.formatNumber(mdailyClaim.getPcov_si_bc())); %></td>
				<td align="center"><%out.print(mdailyClaim.getClmh_place_accident()==null?"":mdailyClaim.getClmh_place_accident()); %></td>
				<td align="center"><%out.print(mdailyClaim.getHospital_name()==null?"":mdailyClaim.getHospital_name()); %></td>
				<td align="center"><%out.print(mdailyClaim.getCause_of_loss()==null?"":mdailyClaim.getCause_of_loss()); %></td>
				<td align="center"><%out.print(mdailyClaim.getInception_date()==null?"":mdailyClaim.getInception_date()); %></td>
				<td align="center"><%out.print(mdailyClaim.getExpire_date()==null?"":mdailyClaim.getExpire_date()); %></td>
				<td align="center"><%out.print(mdailyClaim.getLost_date()==null?"":mdailyClaim.getLost_date()); %></td>
				<td align="center"><%out.print(mdailyClaim.getSettlement_date()==null?"":mdailyClaim.getSettlement_date()); %></td>
				<td align="center"><%out.print(ClsHelper.formatNumber(mdailyClaim.getLoss_100())); %></td>
				<td align="center"><%out.print(ClsHelper.formatNumber(mdailyClaim.getAmount())); %></td>
				<td align="center"><%out.print(ClsHelper.formatNumber(mdailyClaim.getCedant_paid())); %></td>
				<td align="center"><%out.print(ClsHelper.formatNumber(mdailyClaim.getObligatory())); %></td>
				<td align="center"><%out.print(mdailyClaim.getNarration_remark()==null?"":mdailyClaim.getNarration_remark()); %></td>
				<td align="center"><%out.print(mdailyClaim.getClaim_status_desc()==null?"":mdailyClaim.getClaim_status_desc()); %></td>
				<td align="center"><%out.print(mdailyClaim.getReason_of_claim_status()==null?"":mdailyClaim.getReason_of_claim_status()); %></td>
				<td align="center"><%out.print(mdailyClaim.getReported_date()==null?"":mdailyClaim.getReported_date()); %></td>
				<td align="center"><%out.print(mdailyClaim.getApproved_date()==null?"":mdailyClaim.getApproved_date()); %></td>
				<td align="center"><%out.print(mdailyClaim.getTo_date()==null?"":mdailyClaim.getTo_date()); %></td>
				<td align="center"><%out.print(mdailyClaim.getOs_days()==null?"":mdailyClaim.getOs_days()); %></td>
            </tr>
           <%}%>
           </tbody>
           <tfoot>
           </tfoot>
   </table>  
</div>
<input type="hidden" id="txt_rpt_loan_past_due_date" value="<%out.print(""); %>" />
<script type="text/javascript">
	var v_FromDate = $('#rpt_0023_daily_claim_pa_hc_txtDateFrom').val();
	var v_ToDate = $('#rpt_0023_daily_claim_pa_hc_txtDateTo').val();
	var v_delimiter='-';
	var v_title='Policy-Report'+v_delimiter+ v_FromDate +'-TO-'+ v_ToDate;
	$('#rpt_0023_daily_claim_pa_hc_datatable').DataTable( {
		"lengthMenu" : [ [ 200, 50, 25, 10, -1 ],[ 100, 50, 25, 10, "All" ] ],
		//"order": [[ 0, "asc" ]], //desc, asc Columns are ordered using 0 as first column on left. In a five column table the columns are numbered for sorting purposes like this: 0,1,2,3,4,5,6.
		dom: 'Bfrtip',
		"paging": true,
		"autoWidth": true,
		"buttons": [		
		{ extend: 'copy', title:v_title, footer: true },
		//{ extend: 'csv', title:v_title, footer: true },
		{ extend: 'excel', title:v_title, footer: true },
			{
				text: 'PDF',
				extend: 'pdfHtml5',
				filename: v_title,
				orientation: 'landscape', //portrait
				pageSize: 'legal', //A4, A3 , A5 , A6 , legal , letter
				exportOptions: {
				columns: ':visible',
				search: 'applied',
				order: 'applied'
				},
				
				customize: function (doc) {
					//Remove the title created by datatTables
					doc.content.splice(0,1);
					//Create a date string that we use in the footer. Format is dd-mm-yyyy
					var now = new Date();
					var jsDate = now.getDate()+'-'+(now.getMonth()+1)+'-'+now.getFullYear();
					// Logo converted to base64
					// var logo = getBase64FromImageUrl('https://datatables.net/media/images/logo.png');
					// The above call should work, but not when called from codepen.io
					// So we use a online converter and paste the string in.
					// Done on http://codebeautify.org/image-to-base64-converter
					// It's a LONG string scroll down to see the rest of the code !!!
					/* var logo = 'data:image/jpeg;base64,'; */
					// A documentation reference can be found at
					// https://github.com/bpampuch/pdfmake#getting-started
					// Set page margins [left,top,right,bottom] or [horizontal,vertical]
					// or one number for equal spread
					// It's important to create enough space at the top for a header !!!
					doc.pageMargins = [10,10,10,10];
					// Set the font size fot the entire document
					doc.defaultStyle.fontSize = 6;
					// Set the fontsize for the table header
					doc.styles.tableHeader.fontSize = 6;
					// Create a header object with 3 columns
					// Left side: Logo
					// Middle: brandname
					// Right side: A document title
					
					doc['header']=(function() {
						return {
							columns: [
								/* {
									image: logo,
									width: 24
								}, */
								{
									alignment: 'left',
									italics: true,
									text: '',
									fontSize: 5,
									margin: [10,0]
								},
								{
									alignment: 'right',
									fontSize: 5,
									text: '',
									fillColor:"#000"
								}
							],
							tableHeader: {
					            bold:!0,
					            fontSize:6,
					            color:'black',
					            fillColor:'#000',
					            alignment:'center'
					        }
						}
						
					});
				}
			},				
			/* { extend: 'pdf', title:'GL-'+$('#txt_rpt_loan_collection_by_inst_date').val(), footer: true }, */
			//{ extend: 'print', title:v_title, footer: true },
		]
	} );
	
</script> 

<script>
  $(function() {
	$('.chosen-select').chosen();
  });
</script>

<script type="text/javascript">
//START CODE FOR BASIC DATA TABLE 
$(document).ready(function() {
	$('#rpt_0023_daily_claim_pa_hc_datatable').DataTable();
	$('.preloading').fadeOut(500,function(){
		$('.content').fadeIn(500);
	});	
} );
$('#rpt_0023_daily_claim_pa_hc_refresh').click(function(){
	//alert('hello');
	var productCode = $('#rpt_0023_daily_claim_pa_hc_productCode').val();
	var vFromDate = $('#rpt_0023_daily_claim_pa_hc_txtDateFrom').val();
	var vToDate = $('#rpt_0023_daily_claim_pa_hc_txtDateTo').val();
	var msg1 = "pages/rpt-0023-daily-claim-pa-hc.jsp?productCode="+productCode+"&FromDate="+vFromDate+"&ToDate="+vToDate;
	//alert(msg1);
	$("#rpt0023dailyclaimpahc").load(msg1);
});
// END CODE FOR BASIC DATA TABLE

$('#rpt_0023_daily_claim_pa_hc_view_pdf').click(function(){
	//alert('hello');
	var productCode = $('#rpt_0023_daily_claim_pa_hc_productCode').val();
	var vFromDate = $('#rpt_0023_daily_claim_pa_hc_txtDateFrom').val();
	var vToDate = $('#rpt_0023_daily_claim_pa_hc_txtDateTo').val();	
	window.open("rpt0022TreatyReport.htm?fromdate="+vFromDate+"&todate="+vToDate+ "&reportType=1&productcode="+rpt_0023_daily_claim_pa_hc_productCode);
});

$('#rpt_0023_daily_claim_pa_hc_export_excel').click(function(){
	//alert('hello');
	var productCode = $('#rpt_0023_daily_claim_pa_hc_productCode').val();
	var vFromDate = $('#rpt_0023_daily_claim_pa_hc_txtDateFrom').val();
	var vToDate = $('#rpt_0023_daily_claim_pa_hc_txtDateTo').val();	
	window.open("rpt0022TreatyReport.htm?fromdate="+vFromDate+"&todate="+vToDate+ "&reportType=2&productcode="+productCode);
});
/*datetime picker*/
 $('.rpt_0023_daily_claim_pa_hc_txtReportDate').datetimepicker({
        language:  'en',
        weekStart: 1,
        todayBtn:  1,
		autoclose: 1,
		todayHighlight: 1,
		startView: 2,
		minView: 2,
		forceParse: 0
    }); 
</script>


