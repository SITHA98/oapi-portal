...
@Controller
public class rpt_0011_BORDERAUX_MOTOR_Controller extends GenericController {
	...
	@RequestMapping(value = "/rpt0011Borderaux", method = RequestMethod.GET, headers = "Accept=*/*")
...
//		Application_Properties.SERIAL=1;
		try {
			...
			reportEngine.initReport(servletContext.getRealPath("WEB-INF/reports/CAM-RE-REPORTS/BORDERAUX_MOTOR_REPORT.jasper"),
					para);
			...
			if (reportType == 1) {
				...
				reportEngine.exportExcel(outputStream, request, response, "MOTOR");
			}
			int USER_ID = component.USER_ID(request);
